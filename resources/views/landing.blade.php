        ...
        <main class="w-full lg:w-3/4">
            ...
            <section class="relative w-full overflow-hidden rounded-xl mb-8" x-data="{ swiperLoaded: false }">
                <div class="swiper mySwiper rounded-xl overflow-hidden"
                    ...
                    <div class="swiper-button-prev text-white sm:text-transparent"
                        style="color: white; filter: drop-shadow(0px 0px 3px rgba(0,0,0,0.5)); text-shadow: 1px 1px 2px rgba(0,0,0,0.5);">
                    </div>
                    <div class="swiper-button-next text-white sm:text-transparent"
                        style="color: white; filter: drop-shadow(0px 0px 3px rgba(0,0,0,0.5)); text-shadow: 1px 1px 2px rgba(0,0,0,0.5);">
                    </div>
            ...
            <section class="py-8 md:py-12">
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
                    ...
                    'desc' => 'Departemen Elektronika mempelajari dasar-dasar rangkaian listrik, komponen elektronik,
                    dan sistem digital. Juga mencakup perakitan, pemrograman mikrokontroller, serta pengendalian
                    otomatis.'
                    ],
                    [
                    'bg' => 'bg-customRed',
                    ...
                    'desc' => 'Departemen Otomotif mempelajari teori sistem kendaraan seperti mesin, transmisi,
                    kelistrikan, bodi, dan suspensi. Praktiknya dilakukan di bengkel bersama guru pengajar.'
                    ],
                    [
                    'bg' => 'bg-customPink',
                    'title' => 'Pemesinan',
                    ...
                    'desc' => 'Departemen Pemesinan mempelajari teknik pengoperasian mesin produksi seperti bubut,
                    frais, dan CNC. Juga mencakup gambar teknik serta kontrol kualitas produk.',
                    ...
                    'desc' => 'Departemen TIK mempelajari pemrograman, jaringan komputer, teknologi digital, dan
                    kecerdasan buatan. Siswa juga belajar manajemen proyek TI dan etika teknologi.'
                    ]
                    ] as $dept)
                    <div
                        class="{{ $dept['bg'] }} p-4 md:p-5 rounded-xl relative overflow-hidden flex flex-col justify-between transition-all duration-200 hover:shadow-lg">
                        <div class="bg-white p-2 mb-4 rounded-lg">
                            <img class="w-full object-cover max-h-56 rounded-lg {{ $dept['image_class'] ?? '' }}"
                                src="{{ $assetBase }}/assets/{{ $dept['image'] }}" alt="{{ $dept['title'] }} Department"
                        ...
                        <div>
                            ...
                            <a href="/jurusan" class="inline-block">
                                <button
                                    class="bg-white text-[#242424] text-sm md:text-base font-medium py-1.5 px-5 rounded-full hover:bg-gray-100 transition-colors">
                                    Selengkapnya
                                </button>
                            </a>
                        </div>
                        <img class="absolute -bottom-2 -right-6 w-48 opacity-30 -rotate-12"
                            src="{{ $assetBase }}/assets/{{ $dept['bg_image'] }}" alt="" loading="lazy">
                    </div>
                    @endforeach
            ...
            <section class="bg-gray-200 py-8 md:py-12 rounded-xl mt-6 md:mt-8">
                <div class="container mx-auto px-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 justify-items-center">
                        @foreach([
                        ['title' => 'Cek Kelengkapan Seragam', 'image' => 'ceksabuk.jpg', 'desc' => 'Sebelum memasuki
                        sekolah, siswa wajib menunjukan kelengkapan seragam'],
                        ['title' => 'Jalur Hijau', 'image' => 'jalurhijau.jpg', 'desc' => 'SKARIGA selalu membiasakan
                        warga sekolah untuk berjalan di jalur hijau, seperti yang diterapkan di industri'],
                        ['title' => 'Mengaji Pagi', 'image' => 'mengajipagi.jpg', 'desc' => 'Bagi siswa muslim, SKARIGA
                        mengadakan mengaji pagi bersama, seperti membaca juz amma, dan doa pagi. Bagi non Muslim SKARIGA
                        memberi tempat untuk beribadah pagi'],
                        ['title' => 'Makan Bersama', 'image' => 'kebiasaan-4(2).webp', 'desc' => 'Pada istirahat
                        pertama, budaya yang melekat pada murid-murid SKARIGA adalah budaya makan bersama secara
                        lesehan']
                        ] as $habit)
                        <x-habitcard :title="$habit['title']" :image="$habit['image']" loading="lazy">
                            {{ $habit['desc'] }}
                        </x-habitcard>
                        @endforeach
                    </div>
            ...
            <div class="bg-white p-4 md:p-5 rounded-xl shadow-sm">
                ...
                <div id="x-sidenews" class="space-y-4" data-news-container>
                    ...
                    <a href="/berita" class="block mt-4">
                        <button
                            class="w-full bg-customOrange text-white py-2 rounded-lg font-medium hover:bg-customBlue transition-colors">
